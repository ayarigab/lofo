...
(function (global, factory) {
        ...
        : typeof define === 'function' && define.amd
          ...
          : ((global =
                typeof globalThis !== 'undefined'
                    ? globalThis
                    : global || self),
            factory((global.CookieConsent = {})));
})(this, function (exports) {
    'use strict';

    const COOKIE_NAME = 'naaba_cookie';
    ...
    const fetchCategoriesAndServices = (allCategoryNames) => {
        ...
        for (let categoryName of allCategoryNames) {
            ...
            const serviceNames =
                (isObject(services) && getKeys(services)) || [];
    ...
    const retrieveScriptElements = () => {
        ...
        for (const scriptTag of scripts) {
            let scriptCategoryName = getAttribute(
                scriptTag,
                SCRIPT_TAG_SELECTOR,
            );
            ...
            if (elContains(state._allCategoryNames, scriptCategoryName)) {
                ...
                if (scriptServiceName) {
                    const categoryServices =
                        state._allDefinedServices[scriptCategoryName];
    ...
    const resolveEnabledServices = (relativeCategory) => {
        ...
        for (const categoryName of categoriesToConsider) {
            ...
            const readOnlyCategory = elContains(
                _readOnlyCategories,
                categoryName,
            );
             ...
             */
            _acceptedServices[categoryName] = unique(
                _acceptedServices[categoryName],
            );
        }
    };
    ...
    const updateModalToggles = (service, category) => {
        ...
        const servicesInputs =
            globalObj._dom._serviceCheckboxInputs[category] || {};
        const categoryInput =
            globalObj._dom._categoryCheckboxInputs[category] || {};
        ...
        if (isString(service)) {
            if (service === 'all') {
                ...
                if (_preferencesModalExists) {
                    for (let serviceName in servicesInputs) {
                        servicesInputs[serviceName].checked =
                            service === serviceName;
                        dispatchInputChangeEvent(servicesInputs[serviceName]);
                    }
                }
    ...
    const addDataButtonListeners = (
        ...
        const { show, showPreferences, hide, hidePreferences, acceptCategory } =
            api;
        ...
        const showPreferencesModalElements = getElements(
     ...
     * @param {event} [Event]
     */
    const fireEvent = (eventName, modalName, modal, event) => {
        const {
            _onChange,
            _onConsent,
            _onFirstConsent,
        ...
        if (eventName === events._onFirstConsent) {
            ...
            params.changedCategories =
                globalObj._state._lastChangedCategoryNames;
    ...
    const validMimeType = (type) =>
        ['text/javascript', 'module'].includes(type);
    ...
    const manageExistingScripts = (defaultEnabledCategories) => {
        ...
        const loadScriptsHelper = (scripts, index) => {
            ...
            const categoryAccepted = elContains(
                acceptedCategories,
                currScriptCategory,
            );
            const serviceAccepted = currScriptService
                ? elContains(
                      _acceptedServices[currScriptCategory],
                      currScriptService,
                  )
                : false;
            ...
            if (shouldRunScript) {
                ...
                for (const { nodeName } of currScript.attributes) {
                    setAttribute(
                        ...
                        nodeName,
                        currScript[nodeName] ||
                            getAttribute(currScript, nodeName),
                    );
                }
    ...
    const guiManager = (applyToModal) => {
        ...
        const preferencesModalOptions =
            guiOptions && guiOptions.preferencesModal;
    ...
    const createPreferencesModal = (api, createMainContainer) => {
         ...
         */
        const modalData =
            state._currentTranslation &&
            state._currentTranslation.preferencesModal;
        ...
        if (!dom._pmContainer) {
            ...
            addEvent(
                ...
                'keydown',
                (event) => {
                    if (event.keyCode === 27) hidePreferences(event);
                },
                true,
            );
        ...
        sectionsData.forEach((section, sectionIndex) => {
            const sTitleData = section.title,
                ...
                sCurrentCategoryObject =
                    sLinkedCategory &&
                    state._allDefinedCategories[sLinkedCategory],
                sCookieTableData = section.cookieTable,
                sCookieTableBody = sCookieTableData && sCookieTableData.body,
                sCookieTableCaption =
                    sCookieTableData && sCookieTableData.caption,
                sCreateCookieTable =
                    sCookieTableBody && sCookieTableBody.length > 0,
                hasToggle = !!sCurrentCategoryObject,
                /**
                 * @type {Object.<string, import('../global').Service>}
                 */
                sServices =
                    hasToggle && state._allDefinedServices[sLinkedCategory],
                sServiceNames =
                    (isObject(sServices) && getKeys(sServices)) || [],
                sIsExpandableToggle =
                    hasToggle &&
                    (!!sDescriptionData ||
                        !!sCreateCookieTable ||
            ...
            if (sIsExpandableToggle) {
                ...
                if (sCreateCookieTable) {
                     ...
                     */
                    const trHeadFragment =
                        dom._document.createDocumentFragment();
    ...
    function createToggleLabel(
        ...
        /** @type {HTMLSpanElement} */ const toggleIconCircle =
            createNode('span');
        /** @type {HTMLSpanElement} */ const toggleLabelSpan =
            createNode('span');
            ...
            ((value) => {
                addEvent(toggle, CLICK_EVENT, () => {
                    const categoryServicesToggles =
                        dom._serviceCheckboxInputs[value];
            ...
            ((categoryName) => {
                addEvent(toggle, 'change', () => {
                    ...
                    const categoryToggle =
                        dom._categoryCheckboxInputs[categoryName];

                    state._enabledServices[categoryName] = [];

                    for (let serviceName in categoryServicesToggles) {
                        const serviceInput =
                            categoryServicesToggles[serviceName];

                        if (serviceInput.checked) {
                            state._enabledServices[categoryName].push(
                                serviceInput.value,
                            );
                        }
                    }
        ...
        if (!state._invalidConsent) {
            if (isService) {
                ...
                toggle.checked =
                    ...
                    elContains(enabledServices, value);
    ...
    const createConsentModal = (api, createMainContainer) => {
        ...
        if (!dom._cmContainer) {
            ...
            const boxLayout = 'box',
                ...
                consentModalLayout =
                    (consentModalOptions && consentModalOptions.layout) ||
                    boxLayout,
        ...
        if (description) {
            ...
            if (!dom._cmDescription) {
                ...
                dom._cmDescription.className = dom._cmDescription.id =
                    'cm__desc';
    ...
    const setCurrentLanguageCode = (newLanguageCode) => {
        newLanguageCode &&
            (globalObj._state._currentLanguageCode = newLanguageCode);
    };
    ...
    const resolveCurrentLanguageCode = () => {
        ...
        if (autoDetect) {
            debug(
                'CookieConsent [LANG]: autoDetect strategy: "' +
                    autoDetect +
                    '"',
            );
    ...
    const loadTranslationData = async (desiredLanguageCode) => {
        ...
        debug(
            'CookieConsent [LANG]: set language: "' + currentLanguageCode + '"',
        );
    ...
    const getCategoriesWithCookies = (isFirstConsent) => {
        ...
        return categoriesToFilter.filter((categoryName) => {
            const currentCategoryObject =
                state._allDefinedCategories[categoryName];
    ...
    const autoclearCookiesHelper = (isFirstConsent) => {
        ...
        for (const categoryName in state._lastChangedServices) {
            for (const serviceName of state._lastChangedServices[
                categoryName
            ]) {
                const serviceCookies =
                    state._allDefinedServices[categoryName][serviceName]
                        .cookies;
    ...
    const setConfig = (userConfig) => {
        ...
        if (config.hideFromBots === true && nav)
            state._botAgentDetected =
                (nav.userAgent &&
                    /bot|crawl|spider|slurp|teoma/i.test(nav.userAgent)) ||
                nav.webdriver;
        ...
        debug(
            'CookieConsent [CONFIG]: autoClearCookies:',
            config.autoClearCookies,
        );
        debug(
            'CookieConsent [CONFIG]: revision enabled:',
            state._revisionEnabled,
        );
        debug(
            'CookieConsent [CONFIG]: manageScriptTags:',
            config.manageScriptTags,
        );
    ...
    const showPreferences = (event) => {
        ...
        fireEvent(
            globalObj._customEvents._onModalShow,
            PREFERENCES_MODAL_NAME,
            undefined,
            event,
        );
    ...
    const hidePreferences = (event) => {
        ...
        fireEvent(
            globalObj._customEvents._onModalHide,
            PREFERENCES_MODAL_NAME,
            undefined,
            event,
        );
    ...
    const setLanguage = async (newLanguageCode, forceUpdate) => {
         ...
         */
        if (
            newLanguageCode !== getCurrentLanguageCode() ||
            forceUpdate === true
        ) {
    ...
    const reset = (deleteCookie) => {
        ...
        const { name, path, domain, useLocalStorage } =
            globalObj._config.cookie;
